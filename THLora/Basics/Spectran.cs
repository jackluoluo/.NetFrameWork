...
namespace THLora_Testbench
{
    ...
    public class SPUException : ApplicationException
    {
        public SPUException(string message) : base(message) { }
        public SPUException(string message, Exception innerException) : base(message, innerException) { }
    };
    ...
    //=============================================================================================
    /// <summary>
    /// Class for accessing the Aaronia spectrum analyzer.
    /// </summary>
    public class Spectran
    {
        ...
        /* ID of the enviroment variables */
        public const ushort VAR_STARTFREQ = ﻿0x01;     // Start frequency of the sweep in MHz.	 
        public const ushort VAR_STOPFREQ = 0x02;      // Stop frequency of the sweep in MHz. If the Span is 0, the display changes from frequencydomain to time domain. 	 
        public const ushort VAR_RESBANDW = 0x03;      // Resolution bandwidth (enumeration)
        public const ushort VAR_VIDBANDW = 0x04;      // Video bandwidth, the values are the same as RESBANDW.
        public const ushort VAR_SWEEPTIME = 0x05;     // Sweep time in milliseconds.
        public const ushort VAR_ATTENFAC = 0x06;      // Attenuation factor
        public const ushort VAR_REFLEVEL = 0x07;      // Reference level from -20 to 70dB
        public const ushort VAR_DISPRANGE = 0x08;     // Display range from 10 to 100dB
        public const ushort VAR_DISPUNIT = 0x09;      // Display unit
        public const ushort VAR_PULSEMODE = 0x0A;     // Pulse mode – 1 is on, 0 is off.
        public const ushort VAR_DETMODE = 0x0A;       // Detector Mode 0=RMS 1=MIN/MAX
        public const ushort VAR_SENSMODE = 0x0A;      // Sensor Mode
        public const ushort VAR_DEMODMODE = 0x0B;     // Demodulator mode
        public const ushort VAR_SPECPROC = 0x0C;      // Spectrum processing mode program number.
        public const ushort VAR_ANTTYPE = 0x0D;       // Antenna type
        public const ushort VAR_CABLETYPE = 0x0E;     // Cable Type.
        public const ushort VAR_RECVCONF = 0x0F;      // Receiver configuration
        public const ushort VAR_CENTERFREQ = 0x1E;    // Centre Frequency of the sweep in MHz.
        public const ushort VAR_SPANFREQ = 0x1F;      // Modifies STOPFREQ as STARTFREQ + SPANFREQ.
        public const ushort VAR_PREAMPEN = 0x10;      // Internal Pre-amplifier
        public const ushort VAR_SWPDLYACC = 0x11;     // Sweep delay for accuracy mode. 1 enables and 0 disables the delay.
        public const ushort VAR_SWPFRQPTS = 0x12;     // Sweep frequency points request
        public const ushort VAR_USBMEAS = 0x20;       // Measurement data to USB mode
        public const ushort VAR_USBSWPRST = 0x21;     // Reset current sweep.
        public const ushort VAR_USBSWPID = 0x22;      // USB sweep identification request
        public const ushort VAR_USBRUNPROG = 0x23;    // Run SPECTRAN program
        public const ushort VAR_LOGFILEID = 0x30;     // Log. file id. Write log file data to this file (see Program 200).
        public const ushort VAR_LOGSAMPCNT = 0x31;    // Number of Samples to write to log file.
        public const ushort VAR_LOGTIMEIVL = 0x32;    // If the sweep is finished before this interval is reached, the log event isn’t generated.
        public const ushort VAR_SPECDISP = 0x41;      // Spectrum display mode
        public const ushort VAR_PEAKDISP = 0x42;      // Peak display mode.
        public const ushort VAR_MARKMINPK = 0x43;     // Minimum Power to be treated as a marker. 	 
        public const ushort VAR_RDOUTIDX = 0x44;   // The display shows a vertical line at this pixel column and reports the value at this asmarker. The value -1 disables this feature. 	 
        public const ushort VAR_MARKCOUNT = 0x45;     // Marker count 	 
        public const ushort VAR_LEVELTONE = 0x46;     // Peak level audio tone enable / disable	 
        public const ushort VAR_BACKBBEN = 0x47;      // Background BB detector enable / disable	 
        public const ushort VAR_DISPDIS = 0x48;       // Display update disable / enable	 
        public const ushort VAR_SPKVOLUME = 0x49;     // Internal Speaker volume. Floating point range from 0.0 to 1.0	 
        public const ushort VAR_RBWFSTEP = 0x60;      // RBW frequency in MHz, read only. The RBW can be set up by the RESBANDW var.	 
        public const ushort VAR_ANTGAIN = 0x61;       // Nominal antenna gain in dB.	 
        public const ushort VAR_PEAK1POW = 0x80;      // Peak 1 power (Read only).	 
        public const ushort VAR_PEAK2POW = 0x81;      // Peak 2 power (Read only).	 
        public const ushort VAR_PEAK3POW = 0x82;      // Peak 3 power (Read only).	 
        public const ushort VAR_PEAK1FREQ = 0x84;     // Peak 1 freq. (Read only). HF/MHz NF/kHz	 
        public const ushort VAR_PEAK2FREQ = 0x85;     // Peak 2 freq. (Read only).	 
        public const ushort VAR_PEAK3FREQ = 0x86;     // Peak 3 freq. (Read only).	 
        public const ushort VAR_MAXPEAKPOW = 0x90;    // Global maximal peak power	 
        public const ushort VAR_STDTONE = 0xC0;       // Output standard speaker tone. Set a duration value from 1 to 9999	 


        /* Settings */
        public const int USB_RDTIMEOUT = 500;		/* USB read timeout (ms) */
        public const int USB_WRTIMEOUT = 1000;		/* USB write timeout (ms) */
        public const int USB_IDTRIES = 6;		    /* USB identification tries */
        public const int USB_PSTATTRIES = 20;		/* USB prog. status read tries */
        public const int USB_IDCODELEN = 4;		    /* USB ident. code length */

        /* USB commands */
        public const int UCMD_NOP = 0x00;		    /* USB NOP command */
        public const int UCMD_IDREQ = 0x01;		    /* USB identification request */
        public const int UCMD_LOGOUT = 0x02;		/* USB log out */
        public const int UCMD_FILEREAD = 0x10;		/* USB file read command */
        public const int UCMD_FILEWRITE = 0x11;		/* USB file write command */
        public const int UCMD_FILEDEL = 0x12;		/* USB file delete command */
        public const int UCMD_FTMAXSIZE = 0x13;		/* USB file transfer max. size cmd. */
        public const int UCMD_GETSTPVAR = 0x20;		/* UDB get setup variable */
        public const int UCMD_SETSTPVAR = 0x21;		/* UDB set setup variable */
        public const int UCMD_FLSTART = 0xE0;		/* USB flash start command */
        public const int UCMD_FLCLEAR = 0xE1;		/* USB flash clear command */
        public const int UCMD_FLERASE = 0xE2;		/* USB flash erase command */
        public const int UCMD_FLPROG = 0xE3;		/* USB flash program command */
        public const int UCMD_FLEXIT = 0xE4;		/* USB flash loader exit command */
        public const int UCMD_FLPOWOFF = 0xE5;		/* USB flash loader power off cmd. */
        /* USB status replies */
        public const int UST_NOP = 0x00;		    /* USB NOP command */
        public const int UST_IDACK = 0x01;		    /* USB identification ack. */
        public const int UST_FILESTAT = 0x10;		/* USB file handling status */
        public const int UST_FTMAXSIZE = 0x11;		/* USB file transfer max. size */
        public const int UST_FTREADDAT = 0x12;		/* USB file transfer read data */
        public const int UST_GETSTPRES = 0x20;		/* USB get setup var. result */
        public const int UST_SETSTPRES = 0x21;		/* USB set setup var. result */
        public const int UST_AMPFREQDAT = 0x22;		/* USB amplitude frequency data */
        public const int UST_FLMODEREP = 0xE0;		/* USB flash mode report */
        public const int UST_FLSTAT = 0xE1;		    /* USB flash status */
        /* USB identification codes */
        public const int UIDREQ_CODE1 = 0xA5;		/* USB id. request code 1 */
        public const int UIDREQ_CODE2 = 0x5A;		/* USB id. request code 2 */
        public const int UIDREQ_CODE3 = 0xF1;		/* USB id. request code 3 */
        public const int UIDREQ_CODE4 = 0x1F;		/* USB id. request code 4 */
        public const int UIDACK_CODE1 = 0x51;		/* USB id. ack. code 1 */
        public const int UIDACK_CODE2 = 0x1A;		/* USB id. ack. code 2 */
        public const int UIDACK_CODE3 = 0xF5;		/* USB id. ack. code 3 */
        public const int UIDACK_CODE4 = 0xAF;		/* USB id. ack. code 4 */
        /* USB file handling status codes */
        public const int UFST_DONE = 0x00;		    /* File operation done */
        public const int UFST_NOTFND = 0x01;		/* File or section not found error */
        public const int UFST_FSFULL = 0x02;		/* File system full error */
        public const int UFST_ACCERR = 0x03;		/* File system access error */
        /* USB setup variable handling status codes */
        public const int USST_DONE = 0x00;		    /* Setup var. operation done */
        public const int USST_FAILED = 0x01;		/* Setup var. operation failed */
        /* USB flash mode codes */
        public const int UFMODE_APPL = 0x00;		/* USB within application */
        public const int UFMODE_LOADER = 0x01;		/* USB within flash loader */

        /* Resolution bandwidth parameters */
        public const int BANDW_FULL = 0;
        public const int BANDW_3MHZ = 1;
        public const int BANDW_1MHZ = 2;
        public const int BANDW_300KHZ = 3;
        public const int BANDW_100KHZ = 4;
        public const int BANDW_30KHZ = 5;
        public const int BANDW_10KHZ = 6;
        public const int BANDW_3KHZ = 7;
        public const int BANDW_1KHZ = 8;
        public const int BANDW_120KHZ = 100;
        public const int BANDW_9KHZ = 101;
        public const int BANDW_200HZ = 102;
        public const int BANDW_5MHZ = 103;
        public const int BANDW_200KHZ = 104;
        public const int BANDW_1500KHZ = 105;
        ...
        // constructor
        /// <summary>
        /// Constructor for the Spectran class.
        /// </summary>
        public Spectran()
        {
            m_FTDI = new FTDI();
        ...
        /// <summary>
        /// Open the first connected spectrum analyzer.
        /// </summary>
        public void OpenFirst()
        {
            ...
            // Open USB connection
            status = m_FTDI.GetNumberOfDevices(ref count);
            if (status != FTDI.FT_STATUS.FT_OK)
                throw new SPUException("Spectrum Analyzer not installed.");
            if (count < 1)
                throw new SPUException("Spectrum Analyzer not connected.");
            else
            {
                FTDI.FT_DEVICE_INFO_NODE[] devicelist = new FTDI.FT_DEVICE_INFO_NODE[count];
                m_FTDI.GetDeviceList(devicelist);
                int nIndex = -1;
                for (int i = 0; i < count; i++)
                {
                    if (devicelist[i].Description.Contains("SPECTRAN"))
                    {
                        // Aaronia SPECTRAN found
                        nIndex = i;
                        break;
                    }
                }
                if (nIndex >= 0)
                {
                    status = m_FTDI.OpenByIndex((uint)nIndex);
                    if (status != FTDI.FT_STATUS.FT_OK)
                        throw new SPUException("Cannot open spectrum analyzer device.");
                }
                else
                    throw new SPUException("Spectrum Analyzer not found.");
            }

            // Initialize USB connection
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetTimeouts(USB_RDTIMEOUT, USB_WRTIMEOUT);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetResetPipeRetryCount(5);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetFlowControl(FTDI.FT_FLOW_CONTROL.FT_FLOW_NONE, 0, 0);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetDataCharacteristics(FTDI.FT_DATA_BITS.FT_BITS_8, FTDI.FT_STOP_BITS.FT_STOP_BITS_2, FTDI.FT_PARITY.FT_PARITY_NONE);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetBaudRate(921600);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetLatency(1);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.SetCharacters(0, false, 0, false);
            if (status == FTDI.FT_STATUS.FT_OK)
                status = m_FTDI.InTransferSize(0x1000);

            if (status != FTDI.FT_STATUS.FT_OK)
                throw new SPUException("Error during initialization of the spectrum analyzer.");
        ...
        public uint Read(byte[] buffer, uint len)
        {
            FTDI.FT_STATUS status = FTDI.FT_STATUS.FT_OK;
            uint readcount = 0;

            status = m_FTDI.Read(buffer, len, ref readcount);
            if (status != FTDI.FT_STATUS.FT_OK)
                throw new SPUException(string.Format("Spectran.Read: Error ({0}).", status));

            return readcount;
        }

        /// <summary>
        /// Read single command block from input stream.
        /// <param name="cmd">command to scan for</param>
        /// <param name="buffer">receive buffer</param>
        /// <param name="len">count of bytes to read</param>
        /// </summary>
        public uint ReadCmd(byte cmd, byte[] buffer, uint len)
        {
            bool bOK = false;
            FTDI.FT_STATUS status = FTDI.FT_STATUS.FT_OK;
            uint readcount = 1;

            while (!bOK && (readcount > 0))
            {
                // Read command byte
                status = m_FTDI.Read(buffer, 1, ref readcount);
                if (status != FTDI.FT_STATUS.FT_OK)
                    throw new SPUException(string.Format("Spectran.Read: Cannot read command byte ({0}).", status));

                // Continue command processing
                if (readcount == 1)
                {
                    // Get data length
                    // The data length depends from the command type. To keep the communication synchronized, it's
                    // very important to read each block completely.
                    uint datalen = len;
                    switch (buffer[0])
                    {
                        case UST_NOP: datalen = 0; break;
                        case UST_IDACK: datalen = USB_IDCODELEN; break;
                        case UST_FILESTAT: datalen = 1; break;
                        case UST_FTMAXSIZE: datalen = 1; break;
                        case UST_FTREADDAT: datalen = len; break;
                        case UST_FLMODEREP: datalen = 1; break;
                        case UST_GETSTPRES: datalen = 5; break;
                        case UST_SETSTPRES: datalen = 1; break;
                        case UST_AMPFREQDAT: datalen = 16; break;
                        case UST_FLSTAT: datalen = 1; break;
                    }

                    // Read data bytes
                    byte[] buffer2 = new byte[datalen];
                    status = m_FTDI.Read(buffer2, datalen, ref readcount);
                    if (status != FTDI.FT_STATUS.FT_OK)
                        throw new SPUException(string.Format("Spectran.Read: Cannot read data bytes ({0}).", status));
                    readcount++;
        ...
        /// <param name="len">count of bytes to send</param>
        /// </summary>
        public uint Write(byte[] buffer, uint len)
        {
            FTDI.FT_STATUS status = FTDI.FT_STATUS.FT_OK;
            uint numBytesWritten = 0;
            status = m_FTDI.Write(buffer, len, ref numBytesWritten);
            if (status != FTDI.FT_STATUS.FT_OK)
                throw new SPUException(string.Format("Spectran.Write: Cannot write data ({0}).", status));
            return numBytesWritten;
        }

        /// <summary>
        /// Verify and unlock communication.
        /// For safety reasons, the communication is protected. To unlock, the verify command must be send.
        /// </summary>
        public bool Verify()
        {
            byte[] snd1 = { UCMD_IDREQ, UIDREQ_CODE1, UIDREQ_CODE2, UIDREQ_CODE3, UIDREQ_CODE4 };
            byte[] snd2 = { UCMD_NOP };
            byte[] rcv = new byte[5];
            uint written = 0;
            uint rcvcnt = 0;
            ...
            m_FTDI.Purge(FTDI.FT_PURGE.FT_PURGE_RX);
            for (i = 0; !bOK && (i < USB_IDTRIES); i++)
            {
                written = Write(snd1, 5);
                if (written == 5)
                {
                    rcvcnt = Read(rcv, 5);
                    if (rcvcnt != 5)
                    ...
                    bOK = (rcvcnt == 5);
                    bOK &= (rcv[0] == UST_IDACK);
                    bOK &= (rcv[1] == UIDACK_CODE1);
                    bOK &= (rcv[2] == UIDACK_CODE2);
                    bOK &= (rcv[3] == UIDACK_CODE3);
                    bOK &= (rcv[4] == UIDACK_CODE4);
                }
            }
            Thread.Sleep(50);
            m_FTDI.Purge(FTDI.FT_PURGE.FT_PURGE_RX);
        ...
        /// <summary>
        /// Locks the communication.
        /// </summary>
        public void Logout()
        {
            byte[] buf = { UCMD_LOGOUT };
            uint written = Write(buf, 1);
            Thread.Sleep(100);
            m_FTDI.Purge(FTDI.FT_PURGE.FT_PURGE_RX | FTDI.FT_PURGE.FT_PURGE_TX);
            if (written != 1)
                throw new SPUException("Spectran.Logout: error.");
        }

        /// <summary>
        /// Read enviroment variable from spectrum analyzer.
        /// <param name="id">variable id</param>
        /// </summary>
        public double GetStpVar(ushort id)
        {
            float value = 0.0f;
            byte[] buf = { UCMD_GETSTPVAR, 0x00, 0x00, 0, 0, 0 };
            buf[1] = (byte)(id & 0xFF);
            buf[2] = (byte)((id >> 8) & 0xFF);
            uint written = Write(buf, 3);
            uint rcvcnt = 0;
            if (written == 3)
            {
                rcvcnt = Read(buf, 6);
                if (rcvcnt != 6)
                    throw new SPUException(string.Format("Spectran.GetStpVar: read error. Only {0} read (6 bytes expected).", rcvcnt));
                else if (buf[1] != USST_DONE)
                    throw new SPUException(string.Format("Spectran.GetStpVar: No acknoledge, 0x{0:x2} received.", buf[1]));
                else
                    value = BitConverter.ToSingle(buf, 2);
            }
            else
                throw new SPUException(string.Format("Spectran.GetStpVar: write error. Only {0} of 3 bytes written.", written));
            return (double)value;
        }
        ...
        /// </summary>
        public unsafe void SetStpVar(ushort id, double value)
        {
            byte[] buf = { UCMD_SETSTPVAR, 0, 0, 0, 0, 0, 0 };
            // set id
            buf[1] = (byte)(id & 0xFF);
            buf[2] = (byte)((id >> 8) & 0xFF);
            // set value
            ...
            uint ival = *((uint*)&fval);
            buf[3] = (byte)(ival & 0xFF);
            buf[4] = (byte)((ival >> 8) & 0xFF);
            buf[5] = (byte)((ival >> 16) & 0xFF);
            buf[6] = (byte)((ival >> 24) & 0xFF);
            // send command
            uint written = Write(buf, 7);
            uint rcvcnt = 0;
            if (written == 7)
            {
                rcvcnt = ReadCmd(UCMD_SETSTPVAR, buf, 2);
                if (rcvcnt != 2)
                    throw new SPUException(string.Format("Spectran.SetStpVar: read error. Only {0} read (2 bytes expected).", rcvcnt));
                else if (buf[1] != USST_DONE)
                    throw new SPUException(string.Format("Spectran.SetStpVar: No acknoledge, 0x{0:x2} received.", buf[1]));
            }
            else
                throw new SPUException(string.Format("Spectran.SetStpVar: write error. Only {0} of 7 bytes written.", written));
        }

        /// <summary>
        /// Structure of the sweep data items. 
        /// </summary>
        public struct AMPFREQDAT
        {
            public uint time;
            public ulong freq;      // frequency in Hz
            public double minval;   // min. amplitude per sample in dBm
            public double maxval;   // max. amplitude per sample in dBm
        }

        /// <summary>
        /// Get next sweep data item.
        /// <param name="data">item received</param>
        /// </summary>
        public bool GetAmpFreqDat(ref AMPFREQDAT data)
        {
            byte[] buf = new byte[17];
            uint rcvcnt = ReadCmd(UST_AMPFREQDAT, buf, 17);
            if (rcvcnt == 0)
                return false;   // no data!
            else if (rcvcnt != 17)
                throw new SPUException(string.Format("Spectran.GetAmpFreqDat: read error. Only {0} read (17 bytes expected).", rcvcnt));
            else
            {
                data.time = (uint)(buf[1] + (buf[2] << 8) + (buf[3] << 16) + (buf[4] << 24));
                data.freq = (ulong)((buf[5] + (buf[6] << 8) + (buf[7] << 16) + (buf[8] << 24)) * 10);
                data.minval = BitConverter.ToSingle(buf, 9);
                data.maxval = BitConverter.ToSingle(buf, 13);
            }
            return true;
        }
        ...
        /// <param name="fFreqMin">Start frequency in Hz</param>
        /// <param name="fFreqMax">Start frequencyin Hz</param>
        /// <param name="fSweepTime">Duration of the sweep</param>
        /// <param name="nMinSamples">minimal requested sample count</param>
        /// <param name="nBandwidth">Bandwidth in Hz</param>
        /// </summary>
        public void ConfigureSweep(double fFreqMin, double fFreqMax, double fSweepTime, int nMinSamples, int nBandwith)
        {
            try
            {
                SetStpVar(Spectran.VAR_STARTFREQ, fFreqMin / 1E6);
                SetStpVar(Spectran.VAR_STOPFREQ, fFreqMax / 1E6);
            }
            catch (SPUException e)
            {
                throw new SPUException("Configuring the sweep start or stop frequency failed.", e);
            }
            try
            {
                SetStpVar(Spectran.VAR_SWEEPTIME, fSweepTime);
            }
            catch (SPUException e)
            {
                throw new SPUException("Configuring the sweep time failed.", e);
            }
            try
            {
                SetStpVar(Spectran.VAR_SWPFRQPTS, nMinSamples);
            }
            catch (SPUException e)
            {
                throw new SPUException("Configuring the sample count failed.", e);
            }
            try
            {
                SetStpVar(Spectran.VAR_RESBANDW, nBandwith);
            }
            catch (SPUException e)
            {
                throw new SPUException("Configuring the sweep bandwidth failed.", e);
            }
        }

        /// <summary>
        /// Get all items from a complete single sweep
        /// </summary>
        public AMPFREQDAT[] GetSweepData()
        {
            // Start measurement
            SetStpVar(Spectran.VAR_USBMEAS, 1);
            Thread.Sleep(1000);

            // Reset current sweep
            SetStpVar(Spectran.VAR_USBSWPRST, 1);

            // Read the sweep data from one single sweep
            AMPFREQDAT data, data0;
            data.time = 0;
            data.freq = ulong.MaxValue;
            data.minval = 0.0;
            data.maxval = 0.0;
            data0 = data;
            bool bSkip = true;
            ArrayList aSweepData = new ArrayList();
            while (bSkip || (data.freq > data0.freq))
            {
                data0 = data;
                if (!GetAmpFreqDat(ref data))
                    break;
                if (bSkip && (data.freq > data0.freq))
                {
                    aSweepData.Add(data0);
                    bSkip = false;
                }
                if (!bSkip)
                    aSweepData.Add(data);
            ...
            // Stop measurement
            SetStpVar(Spectran.VAR_USBMEAS, 0);

            AMPFREQDAT[] aResult = new AMPFREQDAT[aSweepData.Count];
            for (int i = 0; i < aResult.Length; i++)
                aResult[i] = (AMPFREQDAT)aSweepData[i];
